...
public class PickupObject : TargetObject
{
    ...
    {
        if (((layerMask1.value & 1 << other.gameObject.layer) > 0 && other.gameObject.CompareTag("Player")) || ((layerMask2.value & 1 << other.gameObject.layer) > 0 && other.gameObject.CompareTag("Player")))
        {
            OnCollect();
        }
    }
